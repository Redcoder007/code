...
total_supply = supply;
total_demand = demand;
masked_cost = cost_matrix;
while any(total_supply > 0 ) && any(total_demand > 0)
    [min_val,idx] = min(cost_matrix(:));
    [i,j] = ind2sub(size(cost_matrix),idx);
...
if supply_surplus > 0
    ...
    actual_cost_matrix = masked_cost(:,1:end-1);
elseif demand_surplus > 0
    actual_allocation = allocation_matrix(1:end-1,:);
    actual_cost_matrix = masked_cost(1:end-1 , :);
else
    actual_allocation = allocation_matrix;
    actual_cost_matrix = masked_cost;
end
